...
< !DOCTYPE html >
                    ...
                    <link rel="stylesheet" href="assets/css/variables.css">
                        <link rel="shortcut icon" href="assets/images/8mangos.png">
                            <style>
                                ...
                                .search-box:focus-within {border - color: var(--magenta-main); }
                                ...
                                .search-placeholder .ph-icon {font - size: 48px; margin-bottom: 12px; }
                                ...
                                .results-count strong {color: var(--magenta-glow-claro); }
                                ...
                                .user-card .user-info {flex - grow: 1; overflow: hidden; }
                                .user-card .user-username {font - weight: 700; font-size: 15px; }
                                ...
                                .result-post:hover {border - color: var(--blue-light); }

                                .result-post .post-author {
                                    display: flex;
                                align-items: center;
                                gap: 10px;
                                margin-bottom: 10px;
                                ...
                                .result-post .post-author span {font - size: 12px; color: var(--text-low); }

                                .result-post .post-text {
                                    font - size: 14px;
                                line-height: 1.5;
                                color: var(--text-high);
                                ...
                                .highlight {
                                    background: rgba(209, 44, 125, 0.3);
                                border-radius: 3px;
                                padding: 0 2px;
    ...
    tabPosts.addEventListener('click', () => switchTab('publicaciones'));
        ...
        if (currentQuery.length >= 2) fetchResults(currentQuery, tipo);
    }
    ...
    input.addEventListener('input', () => {
        const q = input.value.trim();
                                ...
                                if (q.length < 2) {
                                    showPlaceholder(q.length === 0 ? 'Escribe al menos 2 caracteres para buscar.' : 'Sigue escribiendo...');
                                return;
        }
        ...
        debounceTimer = setTimeout(() => fetchResults(q, currentTipo), 300);
                                ...
                                function fetchResults(q, tipo) {
                                    fetch(`actions/buscar_ajax.php?q=${encodeURIComponent(q)}&tipo=${tipo}`)
                                        ...
                                        .catch(() => {
                                            spinner.style.display = 'none';
                                            container.innerHTML = '<div class="search-placeholder"><div class="ph-icon">⚠️</div><p>Error al buscar. Inténtalo de nuevo.</p></div>';
                                        });
    }

                                function renderResults({resultados, tipo, query}) {
        if (!resultados.length) {
                ...
                <div class="result-post">
                    <a class="post-author" href="perfil.php?id=${p.id_usuario}">
                        <img src="${p.avatar}" alt="${escHtml(p.username)}">
                        <div>
                            <strong>@${escHtml(p.username)}</strong>
                            <br><span>${formatDate(p.fecha_publicacion)}</span>
                        </div>
                    </a>
                    ${p.contenido_texto ? `<div class="post-text">${highlight(escHtml(p.contenido_texto), query)}</div>` : ''}
                    ${p.media ? `<img class="post-media" src="data:image/jpeg;base64,${p.media}" alt="Imagen">` : ''}
                </div>`;
                                    });
        }
